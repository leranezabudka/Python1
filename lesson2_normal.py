...
date = '02.11.2013'
print('{} {} {} года'.format(day[str(date[0:2])], month[str(date[3:5])], date[-4:]))
...
dict1 = {}
for each in lst:
    dict1.setdefault(str(each), 0)
    if str(each) in dict1.keys():
        dict1[str(each)] += 1
        
for each in dict1.keys():
    if dict1[each] == 1:
        lst2.append(int(each))
